...
public class V1_14_R1 extends V1_13_R2 {
    ...
    protected boolean onLoad(@NotNull Class<? extends Version> version, @NotNull String key, @NotNull Executor executor) {
        if (version == V1_13_R2.class) {
            ...
            if (executor instanceof FieldExecutor) {
                switch (key) {
                    case "Entity:TropicalFish:Variant":
    ...
    public @NotNull ItemStack inventorySubcontainerGetItem(@NotNull InventorySubcontainer inventory, int slot) {
        MethodExecutor method = new MethodExecutor(getClassExec("InventorySubcontainer"), getClassExec("ItemStack"), "getItem", "Gets a item from a InventorySubcontainer", ClassExecutor.INT);
        method.load();

        return new ItemStack(method.invokeInstance(inventory, new IntegerObjExec(slot)));
    }
    public void inventorySubcontainerSetItem(@NotNull InventorySubcontainer inventory, int slot, @NotNull ItemStack item) {
        MethodExecutor method = new MethodExecutor(getClassExec("InventorySubcontainer"), ClassExecutor.VOID, "setItem", "Sets a item of slot at a InventorySubcontainer", ClassExecutor.INT, getClassExec("ItemStack"));
        method.load();

        method.invokeInstance(inventory, new IntegerObjExec(slot), item);
    }
    public @NotNull InventorySubcontainer getHorseInventory(@NotNull AbstractHorse horse) {
        FieldExecutor field = new FieldExecutor(getClassExec("Entity:Horse:Abstract"), getClassExec("InventorySubcontainer"), "inventoryChest", "Gets the horse's inventoryChest");
        field.load();

        return new InventorySubcontainer(field.invokeInstance(horse));
    }
    ...
    public @NotNull Map<String, FieldExecutor> getFields() {
        if (!super.getFields().containsKey("Metadata:Cat:Type")) {
    ...
    public @NotNull VillagerProfessionExec getVillagerDataProfession(@NotNull VillagerData villagerData) {
        MethodExecutor method = new MethodExecutor(getClassExec("VillagerData"), getClassExec("VillagerProfession"), "getProfession", "Gets the profession of a villager data");
        method.load();

        return new VillagerProfessionExec(method.invokeInstance(villagerData));
    }
    public int getVillagerDataLevel(@NotNull VillagerData villagerData) {
        MethodExecutor method = new MethodExecutor(getClassExec("VillagerData"), ClassExecutor.INT, "getLevel", "Gets the level of a villager data");
        method.load();

        //noinspection DataFlowIssue
        return (int) method.invokeInstance(villagerData);
    }
    public @NotNull VillagerType getVillagerDataType(@NotNull VillagerData data) {
        MethodExecutor method = new MethodExecutor(getClassExec("VillagerData"), getClassExec("VillagerType"), "getType", "Gets the type of a villager data");
        method.load();

        return new VillagerType(method.invokeInstance(data));
    }

    protected @NotNull VillagerProfessionExec createVillagerProfessionExec(@NotNull VillagerProfession profession) {
        FieldExecutor executor = new FieldExecutor(getClassExec("VillagerProfession"), getClassExec("VillagerProfession"), profession.getType().name(), "Gets the '" + profession.getType().name().toLowerCase() + "' villager profession instance");
        executor.load();

        return new VillagerProfessionExec(executor.invokeStatic());
    }
